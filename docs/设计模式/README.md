...
4. 懒汉式（线程安全）
    - 解决了线程安全问题
    - 效率太低了，每个线程在想获得类实例时候，执行getInstance()
...
```
每增加 Pizza 的种类(Pepper 披萨) 就需要在 OrderPizza类修改  
![img_6.png](img_6.png)  
#### 静态工厂方法模式  
![img_7.png](img_7.png)  
SimpleFactory类是静态工厂类   
相比与传统模式，把判断逻辑放在新创建的SimpleFactory类而已。 多个不同业务的OrderPizza类 调用同一个SimpleFactory类  
缺陷：这样增加一个业务。就需要创建一个OrderPizza类。后在SimpleFactory类添加逻辑判断，这样违背开闭原则。  
传统模式 也是多个不同业务的OrderPizza类, 对比发现 静态工厂方法模式   就是把判断逻辑抽出来放到SimpleFactory类中  
jdk中Calender类中时间类   
